...
const Navigation = ({ currentPage, onNavigate }: NavigationProps) => {
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className={`lg:hidden p-2 rounded-lg transition-colors ${isScrolled ? 'text-shefa-navy' : 'text-white'
                  }`}
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed inset-0 z-[999] bg-white/95 backdrop-blur-xl transition-transform duration-300 lg:hidden ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        <div className="flex flex-col items-center justify-center h-full gap-6 pt-20">
          {navLinks.map((link) => (
            <button
              key={link.page}
              onClick={() => handleNavigate(link.page)}
              className={`text-2xl font-heading font-medium ${currentPage === link.page ? 'text-shefa-gold' : 'text-shefa-navy'
                }`}
            >
              {link.label}
            </button>
